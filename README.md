...
- **Overview**: local run status, party status, run ID, elapsed time, deaths, active violation count, and a compact party ledger for up to five members. The ledger shows each member's current level and, when their client reports it, level at run join.
