...
Non-death disallowed actions generally create violations. Examples include disallowed bank/auction access, sending mail or taking inbox contents, accepting a trade or trade-window enchant, movement rules, equipped gear rules, and permanently enchanted gear when enchants are disallowed. Opening a mailbox or trade window alone is allowed so accidental clicks or another player initiating trade cannot force violations. Repeated access-window events are throttled so UI reopen spam does not flood the ledger.
