...
- **Overview**: preset/custom run label, local run status, party status, run ID, elapsed time, deaths, active violation count, recent meaningful activity, and a compact party ledger for up to five members. The ledger shows each member's current level and, when their client reports it, level at run join, plus a class-colored badge when class data is available.
...
- **Achievements**: account-level leveling, class, ruleset, and max-level milestones.
