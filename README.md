...
The `Ironman` preset follows the common WoW Challenges shape and allows flight paths. `Iron Vigil` is the stricter Ironman variant with no flight paths and cinematic camera enforced from run start. Preset achievements use the actual starting rule signature, not just the button that was clicked. White/gray-only progression is split so `White Knuckles` requires no self-crafted exemption, while `Self-Forged` tracks white/gray-only runs that keep the self-crafted exemption enabled from start to max level.
