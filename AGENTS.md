...
- `Charter`: start a run, review locked active rules in lightweight native-WoW styled rule groups on a consistent two-column grid, review highlighted rule amendment proposals in the normal rules layout, stop/reset a run, and use one shared action slot that shows Party Sync while party sync work/blockers exist or Modify Rules when the party is synced/local-only; keep `Allow Unsynced Party Members` in Party and Dungeons as the broad grouped-play option for open-world groups, questing, queued Dungeon Finder, manual dungeon groups, and similar real-player party play outside the same Softcore run; keep travel rule tooltips specific about mount-like racial/class movement such as Worgen Running Wild, Druid Travel/Flight Form, and Dracthyr Soar; keep the self-crafted gear exemption as a subordinate gear toggle that only applies while gear restriction is enabled, and lock it off for the Ironman and Iron Vigil presets; keep the Casual preset as the low-restriction grouped baseline (unsynced party members allowed, economy access allowed, no gear restriction, no level-gap enforcement, instanced PvP blocked); keep Chef's Special as the creator's personal preferred preset (grouped white/gray limits, trade/bank allowed, auction/mailbox/warband/guild banks blocked, flying mounts blocked, enchants enabled); keep Iron Vigil as the stricter Ironman preset with no flight paths and cinematic camera enforced; do not use a top detail banner to explain run/proposal state
