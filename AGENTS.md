...
- `Achievements`: account-level leveling, class, ruleset, and max-level milestones in an expandable native-WoW styled journal with category summaries, progress bars, and achievement-specific icons. Non-milestone max-level achievements require starting at level 10 or lower. Preset achievements derive their starting preset from the same discovered rule signature used by Overview and completion awards; Chef's Table, Iron Will, Iron Vigil, and Original Terms additionally require no applied rule amendments. Rule-specific achievements are disqualified only by changing or violating their own tracked rule, so unrelated rule amendments should not block them. Hidden sync/governance/default drift should not create visible per-rule disqualification, but any applied amendment should block no-amendment achievements. Max-level completion creates a per-character clean parchment award popup with concise run statistics, the same discovered run label used by Overview, crisp text, subtle certificate accents, and a restrained in-border stamp; the Achievements tab exposes a hidden-until-earned Completion Award section for reopening it.
