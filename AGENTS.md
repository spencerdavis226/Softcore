...
- Party Sync may also route stale/unsynced display state to a targeted full-state resync without mutating local run state; fresh responses should advance the active Party Sync plan quickly, with retry timers only as fallback. If a run allows unsynced party members, different run IDs, different rules, not-in-run members, and non-addon party members should not create passive party-health flags or local violations during normal play, but addon-enabled peers can still use Party Sync explicitly to align when they choose.
