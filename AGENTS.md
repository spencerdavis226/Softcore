...
- `Achievements`: account-level leveling, class, ruleset, and max-level milestones in an expandable native-WoW styled journal with category summaries, progress bars, and achievement-specific icons. Preset achievements derive their starting preset from the same discovered rule signature used by Overview and completion awards; player-facing rule amendments can disqualify preset/original-terms achievements, but hidden sync/governance/default drift should not. Max-level completion creates a per-character clean parchment award popup with concise run statistics, the same discovered run label used by Overview, crisp text, subtle certificate accents, and a restrained in-border stamp; the Achievements tab exposes a hidden-until-earned Completion Award section for reopening it.
