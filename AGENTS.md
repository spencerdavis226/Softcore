...
- `Overview`: dynamic run label based only on the current rule signature: four exposed preset rule shapes, the hidden exact Head Chef's Special profile, and hidden custom rule profiles, all ending in "Run"; anything else shows **Custom Run**. It also shows an unmodified/last-modified-level rules detail, local run status, party status, elapsed time, deaths, active violation count, recent meaningful activity with character labels when space allows, and a compact five-member party ledger with current/start level, class-colored badge, status, and violation count (no Overview resync control; use `/sc sync` or Charter tab Party Sync)
- `Charter`: start a run, review locked active rules in lightweight native-WoW styled rule groups on a consistent two-column grid, review highlighted rule amendment proposals in the normal rules layout, stop/reset a run, and use one shared action slot that shows Party Sync while party sync work/blockers exist or Modify Rules when the party is synced/local-only; in a party (not raid), the Party Sync control stays visible even while a run proposal or rule amendment is open or before a local run exists—it may be disabled with a tooltip explaining the next step (for example finish or cancel the Charter proposal, then sync); keep `Allow Unsynced Party Members` in Party and Dungeons as the broad grouped-play option for open-world groups, questing, queued Dungeon Finder, manual dungeon groups, and similar real-player party play outside the same Softcore run; keep it enabled by default only for Casual among the four main presets, with Chef's Special, Bronzeman, and Bronze Vigil starting disabled because they are harder profiles; keep travel rule tooltips specific about mount-like racial/class movement such as Worgen Running Wild, Druid Travel/Flight Form, and Dracthyr Soar; keep the self-crafted gear exemption as a subordinate gear toggle that only applies while gear restriction is enabled, and lock it off for the Bronzeman and Bronze Vigil presets; keep the Casual preset as the low-restriction grouped baseline (unsynced party members allowed, economy access allowed, no gear restriction, no level-gap enforcement, instanced PvP blocked); keep Chef's Special as the creator's personal preferred preset (synced party members required by default, grouped white/gray limits, bank allowed, trade/auction/mailbox/warband/guild banks blocked, flying mounts blocked, enchants enabled); keep Head Chef's Special hidden from preset buttons but recognized as Chef's Special with Explorer Mode enabled; keep Bronze Vigil as the stricter Bronzeman preset with no flight paths, cinematic camera, and Explorer Mode enforced; do not use a top detail banner to explain run/proposal state
...
- `Log`: audit history, newest first, in a virtualized compact list capped to the newest 1000 displayable rows for menu responsiveness. The Log tab always exposes CSV export; full rows remain in SavedVariables and the full `/sc export` CSV, while `/sc dl`/`/sc bug` is a bounded diagnostic export for bug reports. UI and `/sc log` hide low-value rows that do not apply to the current ruleset: pet battle start/end; forced-movement start/end audit rows; Explorer Mode enable/restore rows; instance enter unless repeated-dungeon or unsynced-instance rules are restrictive; level-gap notices unless max level gap is restrictive. Applied rule amendments write one **Rules Amended** log row per changed rule (`RULE_AMENDMENT_SUMMARY`) with plain-language names and values (for example `Auction house: restricted (was allowed)`); solo Charter-tab apply and slash `SetRule` skip redundant propose/accept log lines; amendments that apply no real diffs write nothing and do not bump ruleset version.
- `Achievements`: account-level milestones for leveling, max-level runs, preset challenges, class mastery, and rule-family achievements in an expandable native-WoW styled journal with category summaries, progress bars, and unique achievement-specific icons; within each category, unearned entries sort by highest progress first (then definition order and name), and earned entries sort to the bottom (then definition order and newest earn date). Level-milestone rows can show full progress before the account earn is written if the level-up evaluation did not run yet (for example login/reload); `Achievements_CatchUp` runs on entering world with an active run to grant missed milestones. Rule achievements are grouped by access, travel, gear/items, and party/instance restrictions. Non-milestone max-level achievements require starting at level 10 or lower. Characters already at max level cannot start or join a new leveling run; Charter buttons, proposal acceptance, Party Sync join/invite/sync routes, and HUD/tooltip states should show the max-level restriction instead of creating an ineligible run. Preset achievements derive their starting preset from the same discovered rule signature used by Overview and completion awards; Chef's Table, Bronzeman, Bronze Vigil, and Original Terms additionally require no applied rule amendments. Head Chef's Special counts for Chef's Table. Bronze Vigil counts for Bronzeman because it is harder, but Bronzeman does not count for Bronze Vigil. Party Survivor means the run started in group mode and does not imply synced-only party play unless the run rules disable unsynced party members. Closed Circle is the rule-specific-style max-level achievement for keeping the Charter rule `Allow Unsynced Party Members` disabled from start through max level. Rule-specific achievements are disqualified only by changing or violating their own tracked rule, so unrelated rule amendments should not block them. Hidden sync/governance/default drift should not create visible per-rule disqualification, but player-facing rule changes such as `Allow Unsynced Party Members` should be tracked. Max-level completion creates a per-character clean parchment award popup with concise run statistics, the same discovered run label used by Overview, crisp text, subtle certificate accents, and a restrained in-border stamp; the Achievements tab exposes a hidden-until-earned Completion Award section for reopening it.

The HUD is compact and glanceable. It shows local run status when solo, party status/member rows when grouped, and short governance/sync limbo or blocker labels such as Details, Review, Waiting, Settling, Syncing, Invite, Run Sync, No Addon, Offline, Raid Local, Version, Rules, Run ID, Not In Run, Max Level, or Level Gap. Recent rule-governance Settling is grouped-only; solo and raid-local rule edits should return directly to local status after applying. It may appear for pending governance even before a run is active. Clicking the HUD toggles the main menu on the most relevant tab. The minimap button opens the main menu.
...
- Party Sync may also route stale/unsynced display state to a targeted full-state resync without mutating local run state; fresh responses should advance the active Party Sync plan quickly, with retry timers only as fallback. If a run allows unsynced party members, different run IDs, different rules, not-in-run members, non-addon party members, and failed characters from other unsynced runs should not create passive party-health flags or local violations during normal play, but addon-enabled peers can still use Party Sync explicitly to align when they choose. Failed members of the same local Softcore run still obey `failedMemberBlocksParty`.
...
- unsynced party member, when the run does not allow unsynced party members
...
Blockers and conflicts affect party status and display. They should not mutate individual character validity by merely existing; if the local player gains XP or levels while grouped with a blocker, Softcore records the matching local rule violation. When `Allow Unsynced Party Members` is enabled, mixed-party state is kept for Party Sync diagnostics but should not become a passive blocker or violation source by itself. This does not override failed-member blocking for players in the same run, level-gap enforcement, or raid-local behavior. Follower dungeon NPCs should be filtered out of player-party compatibility checks.
...
Run name and preset-shape discovery lives in `RunLabels.lua`. Do not reintroduce duplicate run-label predicates in `Core.lua`; Overview, proposals, exports, completion awards, run start snapshots, and preset achievement eligibility should all call the shared `GetRunDisplayName` / `GetRunLabelForRuleset` / `DetectRulesetPreset` path.
